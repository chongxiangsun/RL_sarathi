...
class Sequence:
    ...
    def __init__(
        ...
        self.arrival_time = arrival_time  # 请求到达时间
        self.first_token_time = None      # 第一个 token 生成时间
        self.token_timestamps: List[float] = []  # 每个 token 生成的时间戳列表
    ...
    def append_token_id(
        ...
        self.record_first_token_time()

        # 记录当前 token 的生成时间
        current_time = time.monotonic()
        if self.token_timestamps:
            # 计算 TBT（相邻 token 的时间间隔）
            tbt = current_time - self.token_timestamps[-1]
    ...
    def record_first_token_time(self):
        """记录第一个 token 的生成时间并计算 TTFT。"""
        if not self.first_token_time:
            self.first_token_time = time.monotonic()
            #ttft = self.first_token_time - self.arrival_time
            #logging.info(f"Request {self.seq_id} TTFT: {ttft:.4f} seconds")
    ...
    def calculate_tbt(self) -> List[float]:
        """计算每两个连续 token 生成的时间间隔列表 (TBT)"""
        return np.diff(self.token_timestamps).tolist()
    
    def calculate_tbt_avg(self) -> float:
        """计算每个任务对应的平均 TBT"""
        tbt = self.calculate_tbt()  # 获取 TBT 列表
        if len(tbt) == 0:
            return 0.0  # 如果没有 token 之间的间隔，返回0，防止空列表
        return np.mean(tbt)  # 返回 TBT 的平均值

    def calculate_tbt_variance_and_std(self) -> Tuple[float, float]:
        """计算 TBT 的方差和标准差"""
        tbts = self.calculate_tbt()
        variance = np.var(tbts) if len(tbts) > 1 else 0.0
        std_dev = np.std(tbts) if len(tbts) > 1 else 0.0
